    ...
    public static void Tokenize(string[] line)
    {
        ...
        for (int l = 0; l < filteredLine.Count; l++)
        {
            ...
            for (int w = 0; w < splitLine.Length; w++)
            {
                ...
                //--------------------------------------------------------------Label Addresses----------------------------------------------
                if (w >= 1 && !Parsers.IsNumeric(splitLine[w]) && !Parsers.IsRegister(splitLine[w]) && splitLine[w].Any(char.IsLetter))  // if its greater than the first word, and everything is letters, its probably a label. p r o b a b l y . . .
                {
